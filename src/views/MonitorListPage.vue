...
const brandTabs = computed(() => {
  const brands = [...new Set(monitorItems.value.map((item) => item.brand).filter(Boolean))]
  return ['ALL', ...brands]
})

const filteredMonitorItems = computed(() => {
  if (activeBrand.value === 'ALL') return monitorItems.value
  return monitorItems.value.filter((item) => item.brand === activeBrand.value)
})
</script>
...
<template>
  <main class="camera-page">
    ...
    <section class="camera-header">
      ...
      <p>Wireless Monitor, On-Camera Monitor, Director Monitor, Monitor ACC</p>
