...
const { getProductByRouteParam } = useCategoryProducts('set', setProducts)

const product = computed(() => getProductByRouteParam(route.params.id))
useProductDetailRoute('set', product)
...
const basePriceForCalc = computed(() => {
  ...
  return Number(product.value?.discountPrice || product.value?.originalPrice || 0)
})
...
watch(
  () => [route.params.id, product.value?.id, product.value?.options],
  async () => {
    ...
    <section class="detail-wrap">
      ...
      <div class="detail-right">
        ...
        <p class="detail-price">
          {{ product.priceDisplayText || formatWon(displayPrice) }}
          <span v-if="hasDiscountPrice && !product.priceDisplayText" class="detail-price-note">SET 구성할인</span>
        </p>
        ...
        <div v-if="hasDetailSelect" class="detail-select-header">
          <h2>DETAIL SELECT</h2>
          <p class="detail-select-note">해당 옵션 선택 후 '복사하기' 버튼을 눌러 카카오톡으로 보내주세요</p>
        </div>
        ...
        <div class="detail-checkout">
          ...
          <div v-if="hasDetailSelect" class="detail-checkout-actions">
            <button type="button" @click="copyOptionSummary">선택 구성 복사하기</button>
            <p class="detail-checkout-note">복사한 구성을 카카오톡으로 전송해주세요</p>
          </div>
        </div>
