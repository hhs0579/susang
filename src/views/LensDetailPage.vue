...
const { getProductByRouteParam } = useCategoryProducts('lens', fallbackLensProducts)
const product = computed(() => getProductByRouteParam(route.params.id))
useProductDetailRoute('lens', product)
    ...
    <section class="detail-wrap">
      ...
      <div class="detail-right">
        ...
        <p v-if="hasDiscountPrice" class="detail-price lens-price-dual">
          정상가 {{ formatWon(product.originalPrice) }}
        </p>
        <p class="detail-price lens-price-dual">
          {{ product.priceDisplayText || `할인가 ${formatWon(displayPrice)}` }}
          <span v-if="hasDiscountPrice && !product.priceDisplayText" class="detail-price-note">카메라 대여시 할인</span>
        </p>
        ...
        <div v-if="hasDetailSelect" class="option-pa nels">
